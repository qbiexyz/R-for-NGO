...
(**下列接續的分析只是挑選變項來示範，可能兩者並沒有太多關係或意義**)
...
以性別(female)為例，呈現男女性的次數分配，並以長條圖呈現
...
查看性別(female)與年齡三組(age_g3)的分布，並以堆疊長條圖、並列長條圖呈現
...
查看年齡(age)和對於生育小孩是盡社會的責任(duty_child)的看法的關係，計算兩者相關係數，並以散布圖呈現
...
查看不同性別(female)，對於生育小孩是盡社會的責任(duty_child)的看法，並以分組盒形圖、分組折線圖呈現

```{r eval=FALSE}
# 分組描述統計
WVS_w7_n4 |>
  group_by(female) |> ## 以female做分組
  summarize(mean = mean(duty_child))

# 畫圖呈現
## 分組盒形圖
ggplot(WVS_w7_n4, aes(
  x = female, y = duty_child)) +
  geom_boxplot()
...
查看不同性別(female)，如何影響對於年齡(age)對於成年子女有責任長期照顧父母(duty_parents)的看法，並以分組散布圖呈現

```{r eval=FALSE}
# 分組相關係數
WVS_w7_n4 |>
  group_by(female) |> ## 以female做分組
  summarize(cor = cor(age, duty_parents, use="complete.obs"))

# 畫圖呈現
## 分組散布圖
ggplot(WVS_w7_n4, aes(x = age, y = duty_parents)) +
  geom_point() +
  geom_smooth() +
