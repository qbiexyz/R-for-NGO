...
顯示age(年齡)排序排序之後前6筆資料的原始位置(由小到大)
...
若只想要查看欄位 id、age(年齡),用`select()` 進行選擇
...
若只想要查看age(年齡)大於等於50的資料，用`filter()`進行篩選
...
若想根據sex(性別)和age(年齡)排序(先排序sex再排age)來查看資料，用`arrange()`進行排序
